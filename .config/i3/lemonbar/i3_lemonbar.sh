...
#  mpc -h 192.168.1.160 | grep volume | awk '{ print $2 }' printf "VOL%d%%    %%\n", $2}}' > "${panel_fifo}" &
#   cnt_vol=0
#printf "VOL%d%%%%\n", $2 > "${panel_fifo}" &
#mpc -h 192.168.1.160 | grep volume | awk '{ print $2 }' > "${panel_fifo}" &

 

#  # GMAIL, "GMA"
#  if [ $((cnt_mail++)) -ge ${upd_mail} ]; then
#    printf "%s%s\n" "GMA" "$(~/.local/bin/gmail.sh)" > "${panel_fifo}"
#    cnt_mail=0
#  fi

  # MPD ################################################## Hacked This
  if [ $((cnt_mpd++)) -ge ${upd_mpd} ]; then
    #printf "%s%s\n" "MPD" "$(ncmpcpp -h 192.168.1.160 --now-playing '{%a - %t}|{%f}' | head -c 60)" > "${panel_fifo}"
    printf "%s%s\n" "MPD" "$(mpc -h 192.168.1.160 -f '[[%artist% - ]%title%]|[%file%]' 2>&1 | head -c 70)" > "${panel_fifo}"
    cnt_mpd=0
  fi
